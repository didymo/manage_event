...
class EventForm extends ContentEntityForm {
  ...
  public function save(array $form, FormStateInterface $form_state) {
    ...
    elseif (!is_null($datetime_member) && is_null($datetime_member_end)) {
      drupal_set_message('Event is not created.', 'error');
      drupal_set_message('End date and time has to be filled.', 'error');
    }
    elseif (!is_null($datetime_member) && !is_null($datetime_member_end) && $datetime_member_end < $datetime_member) {
      drupal_set_message('Event is not created.', 'error');
      drupal_set_message('End Date and Time must be greater than Start Date and Time.', 'error');
    }
    elseif (!is_null($datetime_general) && is_null($datetime_general_end)) {
      drupal_set_message('Event is not created.', 'error');
      drupal_set_message('End date and time has to be filled.', 'error');
    }
    elseif (!is_null($datetime_general) && !is_null($datetime_general_end) && $datetime_general_end < $datetime_general) {
      drupal_set_message('Event is not created.', 'error');
      drupal_set_message('End Date and Time must be greater than Start Date and Time.', 'error');
    }
    else {
      ...
      elseif ($form_state->getValue('end_repeat') == 2) {
      ...
      for ($x = 0; $x <= $diff; $x++) {
        ...
        if (!is_null($datetime_member)) {
          ...
          $node_member->set('rng_status', [
            'value' => 1,
          ]);
          $node_member->set('rng_registration_type', [
            'target_id' => "default_registration",
          ]);
          $node_member->set('rng_registrants_minimum', [
            'value' => 1,
          ]);
          $node_member->set('rng_registrants_maximum', [
            'value' => -1,
          ]);
          $node_member->save();
          $node_member->set('field_message', [
          ...
          $datetime = db_query("SELECT e_datetime_member FROM event WHERE id = :id", [":id" => $entity->id()])->fetchField();
          ...
          $datetime_end = db_query("SELECT e_datetime_member_end FROM event WHERE id = :id", [":id" => $entity->id()])->fetchField();
        ...
        if (!is_null($datetime_general)) {
          ...
          $node_general->set('rng_status', [
            'value' => 1,
          ]);
          $node_general->set('rng_registration_type', [
            'target_id' => "default_registration",
          ]);
          $node_general->set('rng_registrants_minimum', [
            'value' => 1,
          ]);
          $node_general->set('rng_registrants_maximum', [
            'value' => -1,
          ]);
          $node_general->save();
          ...
          $node_general->set('field_message', [
